...
const TextType: React.FC<TextTypeProps> = ({
  ...
  const textArray = useMemo(() => {
    if (Array.isArray(rawInput)) {
      return rawInput.length > 0 ? rawInput : [''];
    }
    return [rawInput];
  }, [rawInput]);

  const activeVariableSpeed = useMemo(() => {
  ...
  const getRandomSpeed = useCallback(() => {
    if (!activeVariableSpeed) return typingSpeed;
    const { min, max } = activeVariableSpeed;
    return Math.random() * (max - min) + min;
  }, [activeVariableSpeed, typingSpeed]);
      ...
      if (isDeleting) {
        ...
        if (currentCharIndex < processedText.length) {
          timeout = setTimeout(
            ...
            activeVariableSpeed ? getRandomSpeed() : typingSpeed
          );
